import { ChevronDown, ChevronRight, Download, FileCode2, FileJson, FileText, RefreshCw } from "lucide-react";
...
export function Editor() {
  ...
  const fetchJob = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({
          error: "Could not load this result. It may have expired.",
        });
      }
    } catch (err: unknown) {
      setStatus({
  ...
  const humanLine = () => {
    if (!activeJobId && !lastOcrJobId)
      return "Run a scan or upload, then OCR — your text shows up here. No IDs to copy.";
    ...
    try {
      ...
      if (res.ok) {
        ...
        const blob = new Blob([blobParts], {
          type: data.media_type || "text/plain",
        });
        ...
        <Card className="lg:col-span-3 bg-slate-900/50 border-slate-800 backdrop-blur-xl flex flex-col min-h-0">
          <CardHeader className="shrink-0">
            <CardTitle className="text-slate-100 flex items-center gap-2">
              <FileText className="w-5 h-5 text-blue-400" /> Text
            </CardTitle>
          ...
          <CardContent className="flex flex-col flex-1 min-h-0 pb-6">
            {status && status.status === "processing" ? (
              ...
              <textarea
                ...
                placeholder={
                  activeJobId
                    ? "Waiting for OCR… or nothing returned yet."
                    : "Go to Scanner or Import, run OCR, then come back — text appears here."
                }
              />
            )}
          </CardContent>
        ...
        <Card className="bg-slate-900/50 border-slate-800 backdrop-blur-xl flex flex-col shrink-0 lg:h-fit">
          <CardHeader>
            <CardTitle className="text-slate-100">
              <Download className="w-5 h-5 inline-block mr-2 text-emerald-400" /> Export
            </CardTitle>
