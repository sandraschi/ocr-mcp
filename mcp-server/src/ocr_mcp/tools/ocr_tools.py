...
def register_ocr_tools(app, backend_manager: BackendManager, config: OCRConfig):
    ...
    async def process_document(
        ...
        """
        Process documents with OCR - handles PDFs, CBZ comics, images, and scanned documents.

        This tool supports multiple advanced OCR backends with specialized capabilities:
        - DeepSeek-OCR: Vision-language model with 4.7M+ downloads
        - Florence-2: Microsoft's unified vision-language model
        - DOTS.OCR: Document structure and table recognition specialist
        - PP-OCRv5: Industrial PaddlePaddle OCR system
        - Qwen-Image-Layered: Advanced image decomposition for complex documents

        Args:
            source_path: Path to source document (PDF file, CBZ file, image file, or scanned image path)
            backend: OCR backend selection:
                - "auto": Automatically select best backend based on content
                - "deepseek-ocr": Vision-language OCR with high accuracy
                - "florence-2": Layout understanding and structured extraction
                - "dots-ocr": Document structure and table specialist
                - "pp-ocrv5": Industrial-grade OCR for production use
                - "qwen-layered": Image decomposition for complex mixed content
                - "got-ocr": Legacy GOT-OCR2.0 backend
                - "tesseract": Legacy Tesseract OCR
            ...
            ocr_mode: OCR processing mode:
                ...
                - "fine-grained": Detailed region-based extraction
            output_format: Output format ("text", "html", "markdown", "json", "xml")
            language: Language code for OCR processing (defaults to auto-detection)
            region: Region coordinates [x1,y1,x2,y2] for fine-grained OCR on specific areas
            enhance_image: Apply automatic image enhancement (deskew, contrast, noise reduction)
            comic_mode: Enable comic book processing optimizations
            manga_layout: Enable manga-specific layout analysis (right-to-left reading, speech bubbles)
            scaffold_separate: Advanced manga mode - scan page structure separately from content panels
            panel_analysis: Analyze individual comic panels with separate OCR processing
            batch_process: Process all pages in multi-page documents (PDF/CBZ)
            save_intermediate: Save intermediate processing results and enhanced images

        Returns:
            Comprehensive OCR processing results with backend-specific information
        """
        logger.info(f"Processing document: {source_path} with backend: {backend}, mode: {ocr_mode}")
        ...
        if ocr_mode == "auto":
            if comic_mode or manga_layout:
                ocr_mode = "format"  # Use formatted OCR for comics/manga
            elif file_type in ["pdf", "cbz", "cbr"]:
                ocr_mode = "format"  # Use formatted OCR for multi-page documents
            else:
                ocr_mode = "text"  # Simple text extraction for single images

        # Process different file types
        if file_type in ["pdf", "cbz", "cbr"]:
            # Multi-page document processing
            return await _process_multi_page_document(
                source_path_obj, file_type, backend, ocr_mode, output_format,
            ...
            return await _process_single_image(
                source_path_obj, backend, ocr_mode, output_format,
                language, region, enhance_image, comic_mode, manga_layout,
