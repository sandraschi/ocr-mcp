...
export function ScanViewer({ imageUrl, onSelectionChange, isProcessing }: ScanViewerProps) {
  ...
  useEffect(() => {
    ...
    // Removed onSelectionChange(null) to prevent loops
    // We only want to clear selection visually when image changes
  }, [imageUrl]);
  ...
  const handleZoom = (delta: number, clientX?: number, clientY?: number) => {
    setScale((prevScale) => {
      ...
      // If zooming with mouse wheel, try to zoom towards cursor
      if (clientX !== undefined && clientY !== undefined && containerRef.current && imageRef.current) {
        const container = containerRef.current.getBoundingClientRect();
        const image = imageRef.current.getBoundingClientRect();

        // Cursor position relative to the container
        const cursorX = clientX - container.left;
        const cursorY = clientY - container.top;

        // Current image center relative to container
        const centerX = image.left - container.left + image.width / 2;
        const centerY = image.top - container.top + image.height / 2;
        ...
        // This is a simplified zoom-to-cursor math that works reasonably well
        // For a perfect implementation, we'd need more complex transform origin math
        if (Math.abs(delta) > 0) {
            // Adjust position slightly when zooming to keep cursor roughly in place
            // Disabled for now as it makes panning jumping if not perfectly calculated
        }
      }
