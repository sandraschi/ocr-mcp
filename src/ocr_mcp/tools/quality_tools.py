...
def register_quality_tools(app, backend_manager: BackendManager, config: OCRConfig):
    """Register all quality assessment tools with the FastMCP app."""
        ...
        try:
            ...
            if isinstance(confidence_scores, list) and confidence_scores:
                confidence_analysis = {
                    ...
                    "confidence_std": round((sum((x - sum(confidence_scores)/len(confidence_scores))**2 for x in confidence_scores) / len(confidence_scores))**0.5, 3),
                ...
                confidence_analysis = {
                    ...
                    "confidence_level": "high" if confidence_scores >= 0.9 else "medium" if confidence_scores >= 0.7 else "low"
                }
            ...
            quality_indicators = {
                ...
                "avg_word_length": round(sum(len(word) for word in ocr_text.split()) / len(ocr_text.split()), 1) if ocr_text.split() else 0
            }
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        quality_assessment = await assess_ocr_quality(
                            result, ground_truth, "comprehensive"
                        )
    ...
    async def analyze_image_quality(
        ...
        if quality_checks is None:
            quality_checks = ["resolution", "contrast", "noise", "blur", "skew", "brightness"]
...
def _calculate_quality_score(confidence_analysis: Dict, quality_indicators: Dict, accuracy_metrics: Optional[Dict] = None) -> int:
...
def _generate_recommendations(quality_score: int, confidence_analysis: Dict, quality_indicators: Dict, backend: str) -> List[str]:
