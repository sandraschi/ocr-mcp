...
def _read_pyproject_desc(repo_path: Path) -> str | None:
    ...
    try:
        ...
        logger.debug("suppressed Exception in _read_pyproject_desc", exc_info=True)
    try:
        import re
...
def _check_port_health_sync(port: int, timeout: float = 1.2) -> dict[str, Any]:
    ...
    for path in (
        ...
        try:
            ...
            logger.debug("suppressed Exception in _check_port_health_sync", exc_info=True)
            continue
    return {
        "port": port,
        "alive": True,
                ...
                for p in line.split():
                    if p.isdigit():
                        try:
                            ...
                            logger.debug("suppressed Exception in _is_process_running", exc_info=True)
        return pids
    except Exception:
        return []
    ...
    async def api_apps_ensure(body: AppsEnsureIn) -> dict[str, Any]:
        ...
        if not health.get("alive") and app_id:
            try:
                ...
                logger.debug("suppressed Exception in api_apps_ensure", exc_info=True)
        if health.get("alive"):
            if app_id:
